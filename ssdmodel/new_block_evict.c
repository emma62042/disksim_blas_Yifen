...
static void ssd_media_access_request_element (ioreq_event *curr)
{
  ...
  if(global_Hint_Queue!=NULL)
  {
    //fprintf(outputssd, "global_Hint_Queue->count=%d\n", global_Hint_Queue->count);
    Hint_page* p = (Hint_page *)malloc( sizeof( Hint_page ) );//分配空間
    //fprintf(outputssd, "global_Hint_Queue:");
    p=global_Hint_Queue->front;
    
    /*while(1)
    {
      if(p==NULL)break;
      //printf("%d|", p->page_num*8);
      fprintf(outputssd, "%d|", p->page_num*8);
      p=p->next;
    }*/
    //printf("\n");
    //fprintf(outputssd, "\n");
  ...
  {
    //printf("global_Hint_Queue=NULL\n");
    fprintf(outputssd, "global_Hint_Queue=NULL\n");
  }
  req_check++;
 ...
 //printf("req_check=%d|blkno=%d\n",req_check,blkno);
  ...
  //printf("curr->arrive_time=%lf\n", curr->arrive_time);
   ...
   if(!(curr->flags&READ))
   {
      if(page_RW_count!=NULL)
      {
        //fprintf(outputssd,"page_RW_count->block_num=%d\n", page_RW_count->page_num*8);
       //fprintf(outputssd,"page_RW_count->r_count=%d|w_count=%d\n", page_RW_count->r_count, page_RW_count->w_count);
      }
...
void add_page_to_cache_buffer(unsigned int lpn,buffer_cache *ptr_buffer_cache)
{
  ...
  /*
  ptr_lru_node = ptr_buffer_cache->ptr_head;
  fprintf(outputssd,"full_cache == LRU___page cache : \n");
  while(1)
  {
    fprintf(outputssd,"%d|", ptr_lru_node->logical_node_num);
    int i=0;
    for(i=0;i<LRUSIZE;i++)
    {
      if(ptr_lru_node->page[i].exist != 0)
        fprintf(outputssd,"[%d]%d:%d|%d, ", i, ptr_lru_node->page[i].lpn*8, ptr_lru_node->page[i].r_count, ptr_lru_node->page[i].w_count);
    }
    fprintf(outputssd,"\n");
    if(ptr_lru_node->next==ptr_buffer_cache->ptr_head)
      break;
    ptr_lru_node=ptr_lru_node->next;
  }
  */
...
void add_a_page_in_the_node(unsigned int logical_node_num,unsigned int offset_in_node,lru_node *ptr_lru_node,buffer_cache *ptr_buffer_cache )
{
    ...
    {
      ptr_lru_node->page[offset_in_node].r_count += page_RW_count->r_count;
      ptr_lru_node->page[offset_in_node].w_count += page_RW_count->w_count;
    }
...
void mark_for_specific_current_block(buffer_cache *ptr_buffer_cache,unsigned int channel_num,unsigned int plane)
{
  ...
  int i;
	...
	while(1)
	{
		...
		if(ptr_buffer_cache->ptr_current_mark_node == ptr_buffer_cache->ptr_head||ptr_buffer_cache->ptr_current_mark_node == ptr_buffer_cache->ptr_head->next)
		{
      fprintf(outputssd, "[TEST]first two node!\n");
      return;
    }
		...
		{	
			...
			ptr_buffer_cache->ptr_current_mark_node = ptr_buffer_cache->ptr_current_mark_node->prev;
			ptr_buffer_cache->current_mark_offset = 0;
      /*fprintf(outputssd,"3186 current_block[%d][%d].ptr_lru_node = %d\n", channel_num, plane, current_block[channel_num][plane].ptr_lru_node->logical_node_num);
			for(i=0;i<64;i++)
      {
        fprintf(outputssd, "[%d]:%d,", current_block[channel_num][plane].ptr_lru_node->page[i].lpn, current_block[channel_num][plane].ptr_lru_node->page[i].exist);
      }
      fprintf(outputssd,"\n");*/
      break;
		}
		...
		{
      fprintf(outputssd,"3292 current_block[%d][%d].ptr_lru_node = %d\n", channel_num, plane, current_block[channel_num][plane].ptr_lru_node->logical_node_num);
      for(i=0;i<64;i++)
      {
        fprintf(outputssd, "[%d]:%d,", current_block[channel_num][plane].ptr_lru_node->page[i].lpn, current_block[channel_num][plane].ptr_lru_node->page[i].exist);
      }
    ...
    if(current_block[channel_num][plane].current_mark_count == )
    {
      fprintf(outputssd,"3292 current_block[%d][%d].ptr_lru_node = %d\n", channel_num, plane, current_block[channel_num][plane].ptr_lru_node->logical_node_num);
      for(i=0;i<64;i++)
      {
...
void kick_page_from_buffer_cache(ioreq_event *curr,buffer_cache *ptr_buffer_cache)
{
	...
	lru_node *ptr_lru_node, *c_node, *r_cnode, *w_cnode, *Szero_node, *LRUzero_node, *temp, *temp2;
	ssd_t *currdisk = getssd(curr->devno);
	curr->tempint2 = 0;
 ...
 //printf("ptr_buffer_cache->total_buffer_page_num=%d, ptr_buffer_cache->max_buffer_page_num=%d\n", ptr_buffer_cache->total_buffer_page_num, ptr_buffer_cache->max_buffer_page_num);
  ...
  //fprintf(outputssd,"in kick_page_from_buffer_cache2222222222222\n");
	/*
  ...
  //printf("block_level_lru_no_parallel=%d\n", block_level_lru_no_parallel);
	 ...
	 * */
  int Total_buffer_page_num = ptr_buffer_cache->total_buffer_page_num;
  //space not enough
  //fprintf(outputssd,"before while Total_buffer_page_num=%d\n",Total_buffer_page_num);
	while(ptr_buffer_cache->total_buffer_page_num > ptr_buffer_cache->max_buffer_page_num)
	{
    //printf(LIGHT_BLUE"in kick_page_from_buffer_cache\n"NONE);
    //printf("ptr_buffer_cache->total_buffer_page_num=%d\n",ptr_buffer_cache->total_buffer_page_num);
    //fprintf(outputssd,"ptr_buffer_cache->total_buffer_page_num=%d\n",ptr_buffer_cache->total_buffer_page_num);
    //fprintf(outputssd,"Total_buffer_page_num=%d\n",Total_buffer_page_num);
    //printf(" > max_buffer_page_num|");
	    /*sh-- our dynamic allocation policy*/
    int i,j,k,m,flagcheck=0,tempc=0,tempp=0;
    //fprintf(outputssdfprintf(outputssd, "lru 64 node channel&plane:\n");
    temp2 = ptr_buffer_cache->ptr_head->prev;//lru's node
    for(i=0;i<64;i++)//from lru node find 64 page
    {
      for(j=0;j<LRUSIZE;j++)//from lru node find 64 page
      {
        if(temp2->page[j].exist == 2 )
        {
         //fprintf(outputssd, "%d[%d][%d],", temp2->logical_node_num, temp2->page[j].channel_num, temp2->page[j].plane);
          flagcheck=0;
          break;
        }
        else
      ...
      if(flagcheck==1)
      {
       //fprintf(outputssd, "%d[N][N],", temp2->logical_node_num);
        flagcheck=0;
      }
      temp2 = temp2->prev;
    }
   //fprintf(outputssd, "\n64 channel&plane:\n");
    for(i=0;i<64;i++)//from lru node find 64 page
    {
        temp2=current_block[tempc][tempp].ptr_lru_node;
        assert(temp2!=NULL);
       //fprintf(outputssd, "#%d[%d][%d],", temp2->logical_node_num, tempc, tempp);
        if(tempp==7)
        {
          tempc++;
          tempp=0;
...
//*******************************************************************************************************
    ...
    bool choose_as_candidate=FALSE;
    for(i=0;i<64;i++)//from lru find 64 node
    {
      /*c_node=current_block[tempc][tempp].ptr_lru_node;
      assert(c_node!=NULL);
      if(tempp==7)
      {
        tempc++;
        ...
        tempp++;*/
      choose_as_candidate=FALSE;
      for(j=0;j<LRUSIZE;j++)//from lru node find 64 page
      {
        if(c_node->page[j].exist == 2)
        {
          choose_as_candidate=TRUE;
          break;
        }
      }
      if(choose_as_candidate==FALSE)
      {
       //fprintf(outputssd,"\t1(choose_as_candidate==FALSE)\n");
        temp=c_node;
        for(k=0;k<64;k++)
        {
          for(m=0;m<LRUSIZE;m++)//from lru node find 64 page
          {
            if(temp->page[m].exist == 2)
            {
              printf("[ptr_node:%d|exit=2] ", temp->logical_node_num);
              break;
            }
          }
          temp=temp->prev;
      ...
      for(j=0;j<LRUSIZE;j++)//from lru node find 64 page
      {
        if(c_node->page[j].exist == 1 || c_node->page[j].exist == 2)
        {
          pagecount++;
          p=global_Hint_Queue->front;
          while(1)
          {
            if(p==NULL)break;
            //find page in hint
            if(c_node->page[j].lpn == p->page_num)
            {
              c_node->hint_repeat++;
              all_repeat++;
              break;
...
//666666666666666666666666666666666666666666666666666666666666666666666666666666666666666666666666666666666666666666666666666
    if(state == -1)//no state 0 // find no repeat
    {
      ...
      for(i=0;i<64;i++)//from lru find 64 node
      {
        ...
        if(choose_as_candidate==FALSE)
        {
         //fprintf(outputssd,"\t2(choose_as_candidate==FALSE)\n");
          temp=c_node;
          for(k=0;k<64;k++)
          {
            for(m=0;m<LRUSIZE;m++)//from lru node find 64 page
        ...
        {
          state = 1;
          double rw_ratio=0, node_rcount=0, node_wcount=0;
          for(j=0;j<LRUSIZE;j++)
          {
            if(c_node->page[j].exist != 0)
            {
              node_rcount = node_rcount + c_node->page[j].r_count;
              node_wcount = node_wcount + c_node->page[j].w_count;
            }
          }
          ...
          else //write intensive //block striping
          {
...
//666666666666666666666666666666666666666666666666666666666666666666666666666666666666666666666666666666666666666666666666666666
    if(state == -1)//repeat>0
    {
      ...
      for(i=0;i<64;i++)//from lru find 64 node
      {
        ...
        if(choose_as_candidate==FALSE)
        {
         //fprintf(outputssd,"\t3(choose_as_candidate==FALSE)\n");
          for(k=0;k<64;k++)
          {
            for(m=0;m<LRUSIZE;m++)//from lru node find 64 page
            {
        ...
        if(c_node->hint_repeat != 0)
        {
          ...
          else//write intensive
          {
            ...
            {
              /*if(pagecount>max_pagecount)//pick size max
              {
                w_cnode = c_node;
                max_pagecount=pagecount;
               //fprintf(outputssd, "cnode=%d, max_pagecount=%d\n", c_node->logical_node_num, max_pagecount);
              }*/
              if(c_node->hint_located < w_cnode->hint_located)//pick hint located most prev
              {
                w_cnode = c_node;
              }
            }
    ...
    //$$$$$$$$$$ state check end $$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$
    // state == 0 | no this block's page in hint
    // state == 1 | have repeat page number == 0, need check r_cnode or w_cnode
    // state == 2 | repeat page number > 0

    // strip_way check
    //printf("----state=%d\n", state);
    int strip_way = 0; // 0:block strip, 1:page strip;
    int seq = 0, seq_temp = 0, block_pcount=0;
    if(state == 0)
    {
      block_pcount=0;
      ptr_lru_node = LRUzero_node;
      ...
      {
        if(seq >= SEQPAGENUM) // seq write //block striping
        {
          strip_way = 0;
        }
        else if(block_pcount > LRUSIZE/RAND_BLOCK_SIZE)// random write //page striping
        {
          strip_way = 0;
        }
        else
    ...
    else if(state == 1)
    {
     ...
     //fprintf(outputssd,"[STATE]state=%d\n",state);
      ...
      else if(locate_r == 100000 && size_w < 100000)//write intensive //block striping
      {
        ...
        else // random write //page striping
        {
          strip_way = 1;
        }
      }
      ...
      {
       //fprintf(outputssd, "locate_r == 10000 && locate_w < 10000 error !!\n");
      }
     //fprintf(outputssd,"[STATE]state=%d|ptr_lru_node=%d\n",state, ptr_lru_node->logical_node_num);
    }
    else if(state == 2)
    {
      block_pcount=0;
      ...
      {
       //fprintf(outputssd, "rep_size == 100000 && all_size < 100000 error !!\n");
      }
     //fprintf(outputssd,"[STATE]state=%d|ptr_lru_node=%d\n",state, ptr_lru_node->logical_node_num);
    }
    else
    {
      fprintf(outputssd,"[BUG]state error, state=%d\n",state);
      assert(0);
    }
//*******strip_way check end**************************************************************************************
    //ptr_lru_node=ptr_buffer_cache->ptr_head->prev;
    for(i=0;i<LRUSIZE;i++)//from lru node find 64 page
    {
   ...
   //fprintf(outputssd, "++++++++++++strip_way check end++++++++++++\n");
...
//**************************************************************************************************
    ...
    {
      //printf("(ptr_lru_node->page[offset_in_node].exist == 2)\n");
      int m;
      if(strip_way==1)//Lg
      {
        //printf("\tstrip_way==1\n");
        int k=0;
        fprintf(outputssd,"!!!!!!!!!strip_way==1|ptr_lru_node->page[offset_in_node]=%d\n", ptr_lru_node->page[offset_in_node].lpn*8);
        kick_Lg_count++;
        int change_ch=0,done=0;

        while(done==0)
        {
          if(current_block[channel_num][plane].current_mark_count == ptr_lru_node->buffer_page_num)
          {
            done=1;
           //fprintf(outputssd, "Lg %d:done=1;\n",ptr_lru_node->logical_node_num);
          } 
          change_ch=0;
          for(i=0;i<LRUSIZE;i++)
          {
            ...
            {
              //printf("simtime=%lf\n", simtime);
              channel_num_Lg = (channel_num+k)%8; 
              plane_Lg = plane;
              //channel_num_Lg = min_response_elem(currdisk);
              //plane_Lg = max_free_page_in_plane(sta_die_num,currdisk,channel_num);
              //ptr_lru_node->page[i].channel_num=channel_num_Lg;
              //ptr_lru_node->page[i].plane=plane_Lg;
             //fprintf(outputssd,"%d.exist=%d\n",ptr_lru_node->page[i].lpn,ptr_lru_node->page[i].exist);
             //fprintf(outputssd,"* if the page already been marked|");
             //fprintf(outputssd,"ptr_lru_node = %d:%d\n", ptr_lru_node->logical_node_num,ptr_lru_node->page[i].lpn);
              //assert(current_block[channel_num][plane].current_mark_count != 0);
             //fprintf(outputssd,"current_block[%d][%d]\n",channel_num_Lg,plane_Lg); 
              //statistic.kick_write_intensive_page_count ++;
              kick_page_striping_page_count++;
              add_to_ioqueue(curr,channel_num_Lg,plane_Lg,ptr_lru_node->page[i].lpn);
              remove_a_page_in_the_node(i,ptr_lru_node,ptr_buffer_cache,channel_num,plane);
              //printf("\tafter remove_a_page_in_the_node|ptr_buffer_cache->total_buffer_page_num=%d\n",ptr_buffer_cache->total_buffer_page_num);
              ptr_buffer_cache->total_buffer_page_num = ptr_buffer_cache->total_buffer_page_num;
              current_block[channel_num_Lg][plane_Lg].flush_w_count_in_current ++;

             //fprintf(outputssd,"ptr_lru_node->buffer_page_num = %d\n",ptr_lru_node->buffer_page_num);
            
              int channel1=channel_num;
              int plane1=plane;
              ...
              {
                mark_for_specific_current_block(ptr_buffer_cache,channel_num,plane);
                temp2=current_block[channel_num][plane].ptr_lru_node;
               //fprintf(outputssd, "next channel node = %d\n", temp2->logical_node_num);
              }
              else
              ...
              if(done==0)
              {
              //fprintf(outputssd,"current_mark_count == 0 && ptr_lru_node->buffer_page_num = %d\n",ptr_lru_node->buffer_page_num);
                for(j=0;j<64;j++)
                {
                  if(current_block[j/8][j%8].ptr_lru_node->logical_node_num==ptr_lru_node->logical_node_num)
                  {
                    ptr_lru_node = current_block[j/8][j%8].ptr_lru_node;
                    offset_in_node = current_block[j/8][j%8].offset_in_node;
          ...
          for(i=0;i<LRUSIZE;i++)
          {
            if(ptr_lru_node->page[i].exist==2)
            {
             ...
             //fprintf(outputssd, "current_block[%d][%d].current_mark_count=%d\n",channel_num,plane,current_block[channel_num][plane].current_mark_count); 
              statistic.kick_write_intensive_page_count ++;
              add_to_ioqueue(curr,channel_num,plane,ptr_lru_node->page[i].lpn);
              remove_a_page_in_the_node(i,ptr_lru_node,ptr_buffer_cache,channel_num,plane);
              //printf("\tafter remove_a_page_in_the_node|ptr_buffer_cache->total_buffer_page_num=%d\n",ptr_buffer_cache->total_buffer_page_num);
              current_block[channel_num][plane].flush_w_count_in_current ++;
      ...
      printf("ptr_lru_node = %d:%d\n", ptr_lru_node->logical_node_num,ptr_lru_node->page[offset_in_node].lpn);
      assert(current_block[channel_num][plane].current_mark_count != 0);
      printf("current_block[channel_num][plane].current_mark_count=%d\n",current_block[channel_num][plane].current_mark_count); 
      statistic.kick_write_intensive_page_count ++;
      ...
      printf("ptr_lru_node=%d ->page[offset_in_node=%d ].exist == 1) \n", ptr_lru_node->logical_node_num, offset_in_node);
			assert(0);
		}
		else
		{
     //fprintf(outputssd,"* else \n");
			assert(ptr_lru_node->page[offset_in_node].exist == 0);//
			current_block[channel_num][plane].offset_in_node ++;
		}
    Total_buffer_page_num = ptr_buffer_cache->total_buffer_page_num;
    //fprintf(outputssd, "////Total_buffer_page_num=%d\n", Total_buffer_page_num);
	}	
}
void kick_read_intensive_page_from_buffer_cache(ioreq_event *curr,unsigned int channel_num,unsigned int plane,buffer_cache *ptr_buffer_cache)
