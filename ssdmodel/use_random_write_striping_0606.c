...
#define SEQPAGENUM 3
#define RWRATIO 0.8
#define RAND_BLOCK_SIZE 8
#define EVICTWINDOW 128
#define HINT0_USELRUSIZE 0 //0:LRU/1:SIZE
...
int check_which_node_to_evict(buffer_cache *ptr_buffer_cache)
{
  ...
  int j,k,m,flagcheck=0,tempc=0,tempp=0;
  ...
  int node_num=-1;
  for(i=0;i<EVICTWINDOW;i++)//from lru find 64 node
  {
      ...
      for(i=0;i<evictwin;i++)//from lru find 64 node
      {
        ...
        if(c_node->hint_repeat != 0)
        {
          ...
          else//write intensive
          {
            ...
            else if(block_pcount > LRUSIZE/RAND_BLOCK_SIZE)
            {
              if(c_node->hint_notrepeat + c_node->hint_repeat < all_size)//find min notrepeat
              {
                all_size = c_node->hint_notrepeat + c_node->hint_repeat;
    ...
    block_pcount=0;
      ...
      for(j=0;j<LRUSIZE;j++)
      {
        if(ptr_buffer_cache->ptr_current_mark_node->page[j].exist != 0)
        {
          ...
          if(seq_temp>seq)
            seq = seq_temp;
          node_rcount = node_rcount + ptr_buffer_cache->ptr_current_mark_node->page[j].r_count;
          node_wcount = node_wcount + ptr_buffer_cache->ptr_current_mark_node->page[j].w_count;
          if(ptr_buffer_cache->ptr_current_mark_node->page[j].w_count == 0)
          {
            fprintf(outputssd,"ptr_buffer_cache->ptr_current_mark_node->page[j].w_count error\n");
          }
        }
        else
        {
      ...
      else //write intensive
      {
        if(seq >= SEQPAGENUM) // seq write //block striping
        {
          fprintf(outputssd, "ptr_buffer_cache->ptr_current_mark_node= %d, seq=%d |strip_way=0\n",ptr_buffer_cache->ptr_current_mark_node->logical_node_num, seq);
          strip_way = 0;
        }
        else if(block_pcount > LRUSIZE/RAND_BLOCK_SIZE)// random write //page striping
        {
          fprintf(outputssd, "ptr_buffer_cache->ptr_current_mark_node= %d, seq=%d, block_pcount=%d|strip_way=0\n",ptr_buffer_cache->ptr_current_mark_node->logical_node_num, seq, block_pcount);
          strip_way = 0;
        }
        else
        {
          fprintf(outputssd, "ptr_buffer_cache->ptr_current_mark_node= %d, rw_ratio=%lf, seq=%d, block_pcount=%d |strip_way=1\n",ptr_buffer_cache->ptr_current_mark_node->logical_node_num, rw_ratio, seq, block_pcount);
          strip_way = 1;
          //strip_way = 0;
        }
      }
...
void show_result(buffer_cache *ptr_buffer_cache)
{
  ...
  statistic_the_data_in_every_stage();
  printf(LIGHT_GREEN"\n[YIFEN] SEQPAGENUM=%d, RWRATIO=%lf, RAND_BLOCK_SIZE=%d, EVICTWINDOW=%d, HINT0_USELRUSIZE=%d\n"NONE, SEQPAGENUM, RWRATIO, RAND_BLOCK_SIZE, EVICTWINDOW, HINT0_USELRUSIZE);
  
  printf(LIGHT_BLUE"\n[YIFEN] State0=%d, State1=%d, State2=%d\n"NONE, State0, State1, State2);
  printf(LIGHT_BLUE"[YIFEN] kick_page_striping_page_count=%d, kick_block_striping_page_count=%d\n"NONE, kick_page_striping_page_count, kick_block_striping_page_count);
  printf(LIGHT_BLUE"[YIFEN] kick_Lg_count = %d kick_Pg_count = %d \n"NONE,kick_Lg_count,kick_Pg_count);
